...
(go run ./cmd/web/ --backend "http://localhost:8081" --backend http://localhost:8082 --reload --llm http://localhost:11434 --db $POSTGRES_URL --cf_client_id $CF_CLIENT_ID --cf_client_secret $CF_CLIENT_SECRET) &
